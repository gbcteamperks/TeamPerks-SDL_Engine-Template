...
void SoundManager::unload(std::string id, sound_type type)
{
...
void SoundManager::playMusic(std::string id, int loop/* = -1 */, int fadeIn/* = 0 */)
{	
	std::cout << "Playing music..." << fadeIn << std::endl;
	if (Mix_FadeInMusic(m_music[id], loop, fadeIn) == -1)
		std::cout << "Unable to play music: ERROR - " << Mix_GetError() << std::endl;
}

void SoundManager::stopMusic(int fadeOut/* = 0 */) const
{
	if (Mix_PlayingMusic())
...
void SoundManager::playSound(std::string id, int loop/* = 0 */, int channel/* = -1 */)
{
...
void SoundManager::panSet(const int amount, const int channel)
{
	...
	int leftVol = (255 * std::min((m_pan - 100.0) / (50.0 - 100.0), 1.0)); // z-min/max-min
	std::cout << "Left:" << leftVol << std::endl;
	int rightVol = (255 * std::min(m_pan / 50.0, 1.0));
